...
export interface StudyTaskRefInterface {
	enum: eStudyTaskOptions;
	_id: Types.ObjectId;
	tag: eStudyTaskOptionsChapterTags | eStudyTaskOptionsTopicTags;
}
...
const StudyTaskRefSchema = new Schema<StudyTaskRefInterface>(
	{
		enum: {
			type: String,
			enum: Object.values(eStudyTaskOptions),
			required: true,
		},
...
async function assertValidStudyTaskRef(
	ref: StudyTaskRefInterface,
): Promise<void> {
	if (ref.enum === eStudyTaskOptions.Chapter) {
		if (
			!Object.values(eStudyTaskOptionsChapterTags).includes(
				ref.tag as eStudyTaskOptionsChapterTags,
			)
		) {
			throw new Error(
				`Invalid tag "${ref.tag}" for a chapter-type study-task. Allowed: ${Object.values(eStudyTaskOptionsChapterTags).join(', ')}`,
			);
		}
	...
	if (ref.enum === eStudyTaskOptions.Topic) {
		if (
			!Object.values(eStudyTaskOptionsTopicTags).includes(
				ref.tag as eStudyTaskOptionsTopicTags,
			)
		) {
			throw new Error(
				`Invalid tag "${ref.tag}" for a topic-type study-task. Allowed: ${Object.values(eStudyTaskOptionsTopicTags).join(', ')}`,
			);
		}
...
StudyTasksSchema.pre(
	...
	function () {
		...
		const touchesRef =
			'studyTask' in rawUpdate ||
			Boolean(rawUpdate.$set && 'studyTask' in rawUpdate.$set);
		if (touchesRef) {
			throw new Error(
				'studyTask reference (enum/_id/tag) is immutable after creation.',
			);
		}
	},
);
...
const StudyTaskModel =
	(mongoose.models
		.StudyTask as mongoose.AggregatePaginateModel<StudyTasksModelInterface>) ||
	mongoose.model<
		StudyTasksModelInterface,
		mongoose.AggregatePaginateModel<StudyTasksModelInterface>
	>('StudyTask', StudyTasksSchema);
